...
class Killbot {
    ...
    function check() {
        ...
        $sql = "SELECT ip, type FROM visitorip WHERE ip = '$ip'";
        $result = $link->query($sql);
        if($result->num_rows > 0) {
            $row = $result->fetch_assoc();
            if($row["type"] == "bot")   return true;
            else return false;
        }
        ...
        if($json['meta']['code'] == 200) {
            if($json['data']['block_access'] == true) {
                ...
                $sql = "INSERT INTO visitorip (ip, type) VALUES ('$ip', 'bot')";
                $link->query($sql);
                return true;
            } else {
                $this->logs($ip, 'Granted by Killbot', 'granted');
                $sql = "INSERT INTO visitorip (ip, type) VALUES ('$ip', 'user')";
                $link->query($sql);
                return false;
            }
        } else {
